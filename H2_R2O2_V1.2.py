...
production_rate = 11.42  # kg regolith/hours (11.42 kg/hour = 100 t/year)
...
# Later must substract unreacted ilmenite
R_out_water_mols = B_out_ilmenite_mols
...
# change total energy to total_energy_pre_batch
Energy_required_per_month = batches_per_month * Total_energy
...
print("  ")
print("Energy per kg rego input (kWh/kg): ",
      round(Total_energy/X_out_regolith, 2))

print("Avg PV KwH/m2/month: ", round(PV_out_kwh_per_m2_month, 2))
print("batch_per_month ", production_rate*24*30)
print("m2 of PV required for the production: ",
      round(PV_area_m2_required_for_production, 2))
print("En demand per month in kWh: ", round(Energy_required_per_month, 0))
print("Total Monthly Prod LOX kg: ", round(
    total_monthly_LOX_Stored_final_kg, 0))
...
if forloops == True:
    ...
    for i in range(1, max_loops):
        ...
        # increasing variable
        pre_benef_ilmenite_grade = pre_benef_ilmenite_grade + \
            pre_benef_ilmenite_grade/max_loops
        ...
        # report result
        print("ilmen: ", round(pre_benef_ilmenite_grade*100, 0), "%.",
              "  Energy-req kWh/kg-LOX: ", round(Total_energy/S_out_dioxy_kg, 2))
